...
As noted above, `Same/Different Sense` was determined by consulting Merriam-Webster and the OED. There were several cases in which it was difficult to tell whether two usages were in fact different senses (e.g., *glossy magazine* and *weekly magazine*); these were marked `Unsure` under `Ambiguity Type`. These will ultimately be excluded from these analyses and from future experiments (as well as the publicly available normed relatedness judgments).
...
The minimum number of observations for any given pair is `r min(df_norms_final$count)`, and the median number of observations is `r median(df_norms_final$count)`.
...
```{r}
df_norms_final = read_csv("../../data/stims/item_means.csv")
df_norms_final = df_norms_final %>%
...
ggsave("../../Figures/norms_no_homonymys.pdf", dpi = 300)
...
ggsave("../../Figures/norms_no_homonymys_hist.pdf", dpi = 300)
...
ggsave("../../Figures/distances_no_homonyms_dens.pdf", dpi = 300)
```
...
df_norms_final %>%
  ...
  geom_density(alpha = .7,
               size = 1.2) +
  labs(x = "Residuals (relatedness ~ BERT)",
       y = "Density",
       fill = "Same Sense") +
  geom_vline(xintercept = 0, size = 1.2, linetype = "dotted") +
  theme_minimal() +
  theme(axis.title = element_text(size=rel(2)),
        axis.text = element_text(size = rel(2)),
        legend.text = element_text(size = rel(2)),
...
ggsave("../../Figures/resids_no_homonyms.pdf", dpi = 300)
...
ggsave("../../Figures/resids_no_homonyms_hist.pdf", dpi = 300)

```

# Supplementary: Classifying 'unsure' items

For several items, it was unclear in the dictionaries whether the `different` sense usages were actually different senses. We excluded these from the final RAW-C dataset, but here, we ask whether one can use `mean relatedness` to classify allegedly `different sense` usages as same vs. different senses.

```{r}
table(df_merged_all$ambiguity_type)

df_norms_final_all = df_merged_all %>%
  group_by(word, same, ambiguity_type, version, Class) %>%
  summarise(mean_relatedness = mean(relatedness),
            count = n(),
            sd_relatedness = sd(relatedness),
            distance_bert = mean(distance_bert))
table(df_norms_final_all$ambiguity_type)
nrow(df_norms_final_all)
```


We can classify `Unsure`, `different sense` pairs using logistic regression, with the input being either `cosine distance` or `mean relatedness`.

```{r}
# All items for which we *are* sure
df_norms_final_all_no_unsure = df_norms_final_all %>%
  filter(ambiguity_type != "Unsure")
```

Here, we train using relatedness judgments.

```{r}
# Train model using relatedness
model_relatedness = glm(data = df_norms_final_all_no_unsure,
                     same ~ mean_relatedness,
                     family = binomial())

df_norms_final_all$relatedness_prob= predict(model_relatedness, df_norms_final_all, type = "response")
df_norms_final_all = df_norms_final_all %>%
  mutate(relatedness_prediction = ifelse(relatedness_prob > .5, TRUE, FALSE)) %>%
  mutate(relatedness_correct = relatedness_prediction == same)

df_norms_final_all %>%
  ggplot(aes(x = relatedness_prob)) +
  geom_histogram(bins = 15,
                 aes(y = ..density..),
                 alpha = .5) +
  facet_wrap(~same + ambiguity_type)
...
df_norms_final_all %>%
  group_by(ambiguity_type, same) %>%
  summarise(accuracy = mean(relatedness_correct),
            prop_same = mean(relatedness_prediction),
            count = n())

df_no_unsure = df_norms_final_all %>%
  filter(ambiguity_type != "Unsure")
mean(df_no_unsure$relatedness_correct)
```
